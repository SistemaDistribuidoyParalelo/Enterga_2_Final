...
int main(int argc,char*argv[]){
    ...
    tiempoComunicacion = dwalltime() - tiempoComunicacion;
    printf("ID = %d  | El tiempo de ejecucion = %f \n",ID , tiempoComunicacion);

    MPI_Gather(parcialAB,(N*N)/T,MPI_DOUBLE,M,(N*N)/T,MPI_DOUBLE,0,MPI_COMM_WORLD);

    tiempoInicioCompleto = dwalltime() - tiempoInicioCompleto;
    printf("ID = %d  | Tiempo comunicacion %f\n", ID, tiempoInicioCompleto - tiempoComunicacion);
    printf("ID = %d  | Tiempo total %f\n", ID, tiempoInicioCompleto);
